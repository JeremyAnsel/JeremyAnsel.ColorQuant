...
packages\coverlet "JeremyAnsel.ColorQuant.Tests\bin\%Configuration%\netcoreapp3.0\JeremyAnsel.ColorQuant.Tests.dll" --target "dotnet" --targetargs "test -f netcoreapp3.0 --no-build" --output "bld\coverage\results-netcoreapp3.0.xml" --format opencover --use-source-link
if %ERRORLEVEL% neq 0 exit /b %ERRORLEVEL%

packages\coverlet "JeremyAnsel.ColorQuant.Tests\bin\%Configuration%\net452\JeremyAnsel.ColorQuant.Tests.dll" --target "dotnet" --targetargs "test -f net452 --no-build" --output "bld\coverage\results-net452.xml" --format opencover --use-source-link
if %ERRORLEVEL% neq 0 exit /b %ERRORLEVEL%
